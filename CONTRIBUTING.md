...
- 子问的分值同样用 `score` 参数传入，由独立组件在题目中显示，不写入 `stem`。
...
- 线条样式统一使用 `figure-style`，在 `src/figure.typ` 中调整；共享边、坐标轴和网格避免重复描线。
- 插图用 `#figure(...)` 包裹，题目中默认左对齐，解析中默认居中。
- 绘图函数放在对应试卷中，顺序为“导入 → `exam` 配置 → 绘图函数 → 题目”。
